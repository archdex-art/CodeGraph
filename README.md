...
| **Test suite** | **88 test files** in the workspace (security, indexer, scoring, pillars, coverage, dependencies, codeintel, incremental indexing, executor, verify gates, orchestrator, specialists, migrations, tenant-isolation, CLI, README claims, and more). 1,245 cases as of 2026-08-05 — the file count is asserted by a test, the case count is a point-in-time figure that moves with every commit | `npm run test` |
