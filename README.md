...
| What | Result | Source |
|---|---|---|
| **Symbol graph extraction** (`expressjs/express@a371447`) | 174 symbols across 159 files plus 110 synthetic module nodes; **261 resolved call edges**; 0 call cycles. Symbols with no inbound edge: **107 of 174 (61%)**, down from 155 (89%) — call-site attribution used to require a named enclosing function, which discarded 46% of resolved calls and reported the targets as dead code. Precision is separately measured on this repository against the TypeScript checker as ground truth: **98.4%** over 2,159 calls where both resolvers answered. The remaining 61% is a mix of genuinely-uncalled exports and edges still missed; sampled at 63/38 respectively, so it is not all defect | `npm run bench` |
| **Agent swarm** (`expressjs/express@a371447`) | 74 findings across 6 active specialists (P0:25 · P1:15 · P2:34 · P3:0); Health Score 77, *simulated* **77 → 86** if P0+P1 are fixed. The projection re-runs the real scorer over the issues that would remain, so it simulates the shipped model rather than estimating — but it is a simulation, not a measurement. The measured result is the row below | `npm run bench` |
...
| **Test suite** | **63 test files** in the workspace and **8** for the Electron app, plus a Playwright e2e spec run separately (security, indexer, scoring, pillars, coverage, dependencies, codeintel, executor, verify gates, orchestrator, specialists, migrations, tenant-isolation, CLI, README claims, and more). 842 and 35 cases respectively as of 2026-07-30 — the file counts are asserted by a test, the case counts are a point-in-time figure that moves with every commit | `npm run test`; `npm test --workspace @codegraph/desktop` |
