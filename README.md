...
| **Test suite** | **90 test files** in the workspace (security, indexer, scoring, pillars, coverage, dependencies, codeintel, graph scope, wheel-zoom policy, incremental indexing, executor, verify gates, orchestrator, specialists, migrations, tenant-isolation, CLI, README claims, and more). 1,266 cases as of 2026-08-07 — the file count is asserted by a test, the case count is a point-in-time figure that moves with every commit | `npm run test` |
