...
| **Metrics** | Exposed at `/api/metrics` (Prometheus text format). **Implemented:** `cg_run_total{outcome}`, `cg_findings_total{rule,severity}`, `cg_verification_total{gate,outcome}` (counters, SQLite-backed so they survive restart and several processes); `cg_cache_hit_ratio`, `cg_queue_depth` (gauges, sampled at scrape — a stored gauge serves whatever was true when some process last wrote it). `cg_stage_duration_seconds{stage}` (summary — `_sum`/`_count`, so `rate(sum)/rate(count)` gives the mean per stage). **No buckets, so no percentiles:** bucket boundaries would have to be chosen from evidence nobody has yet, and arbitrary ones produce confident wrong quantiles. |
| **Traces** | OpenTelemetry spans per stage, parented to the run. Off unless `OTEL_EXPORTER_OTLP_ENDPOINT` is set. |
| **Run record** | Degradations are persisted (`ScanCoverage`, including per-tier LOC from §8.3) and every run now returns `stageTimings` — `scan`, `imports`, `dependencies`, `detect`, `score`, `symbol-graph`, named for the packages LLD §13 split out, so a slow run points at a package rather than at "indexing". Measured on this repository: symbol-graph 1,233ms, detect 822ms, scan 35ms. Timing is returned from the pipeline rather than pushed to a store, because `analysis` sits below `persistence`. |
