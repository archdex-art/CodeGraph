...
export async function hasTypeConfig(root: string): Promise<boolean> {
  // `isFile()`, not `existsSync`: the latter is true for a DIRECTORY named tsconfig.json, and
  // gate 2 would then run `tsc --noEmit` against a project with no real config and report a
  // confusing failure instead of an honest skip. Found by a test, not by reading.
  try {
