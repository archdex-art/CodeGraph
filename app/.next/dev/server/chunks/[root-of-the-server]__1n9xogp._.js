...
"[project]/src/lib/codeintel/query.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
class QueryEngine {
    ...
    constructor(graph){
        ...
        this.byId = new Map();
        this.byName = new Map();
        this.out = new Map(); // source -> edges
        this.inc = new Map(); // target -> edges
    ...
    /** Fuzzy symbol search by name/signature/tag, ranked. */ search(q, limit = 30) {
    ...
    /** Callees: symbols this one calls. */ callees(id) {
        return (this.out.get(id) || []).filter((e)=>e.kind === "calls").map((e)=>this.byId.get(e.target)).filter((s)=>!!s);
    }
    /** Callers: symbols that call this one. */ callers(id) {
        return (this.inc.get(id) || []).filter((e)=>e.kind === "calls").map((e)=>this.byId.get(e.source)).filter((s)=>!!s);
    }
    /** Members of a container (class -> methods). */ members(id) {
        return (this.out.get(id) || []).filter((e)=>e.kind === "contains").map((e)=>this.byId.get(e.target)).filter((s)=>!!s);
    }
    /** Impact set: transitive callers up to depth (who breaks if this changes). */ impact(id, depth = 3) {
        const seen = new Set([
            id
        ]);
        let frontier = [
    ...
    /** Dead code: exported-or-not symbols with zero resolved callers and not entrypoints. */ deadCode() {
        return this.graph.symbols.filter((s)=>(s.kind === "function" || s.kind === "method") && s.fanIn === 0 && !/^(main|default|handler|index|render|app|page|route|get|post|put|delete|constructor)$/i.test(s.name) && !s.tags.includes("test"));
    }
    /** Detect call cycles (SCCs of size>1 or self-loops) via DFS. */ cycles(maxReport = 20) {
        const index = new Map();
        const low = new Map();
    ...
    /** Hub symbols: highest connectivity (fanIn+fanOut). */ hubs(limit = 15) {
        return [
            ...this.graph.symbols
        ].sort((a, b)=>b.fanIn + b.fanOut - (a.fanIn + a.fanOut)).slice(0, limit);
    }
    outEdges(id) {
...
"[project]/src/lib/agents/specialists.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const security = {
    ...
    label: "Security",
    run ({ repo }) {
        return repo.issues.filter((i)=>i.dimension === "security").slice(0, 40).map((i)=>mk("security", {
                severity: i.severity,
...
const refactor = {
    ...
    label: "Refactor",
    run ({ repo }) {
        return repo.issues.filter((i)=>i.dimension === "maintainability" && /large file/i.test(i.title)).slice(0, 20).map((i)=>mk("refactor", {
                severity: i.severity,
                confidence: 0.7,
                title: i.title,
...
const deadcode = {
    ...
    label: "Dead code",
    run ({ qe }) {
        return qe.deadCode().slice(0, 25).map((s)=>mk("deadcode", {
                severity: 2,
                confidence: 0.5,
...
const dependency = {
    ...
    label: "Dependency",
    run ({ repo }) {
        return repo.issues.filter((i)=>i.dimension === "dependency_hygiene").slice(0, 25).map((i)=>mk("dependency", {
                severity: i.severity,
                confidence: 0.8,
                title: i.title,
...
const architecture = {
    ...
    label: "Architecture",
    run ({ qe }) {
        return qe.cycles(15).map((cycle)=>mk("architecture", {
                severity: 3,
                confidence: 0.75,
...
const test = {
    ...
    label: "Test coverage",
    run ({ repo }) {
        return repo.issues.filter((i)=>i.dimension === "test_integrity").slice(0, 15).map((i)=>mk("test", {
                severity: i.severity,
                confidence: 0.7,
                title: i.title,
...
"[project]/src/lib/agents/orchestrator.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$codeintel$2f$query$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/codeintel/query.ts [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$agents$2f$specialists$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/agents/specialists.ts [app-route] (ecmascript)");
;
;
function runSwarm(repo) {
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$agents$2f$specialists$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["resetSeq"])();
    const qe = new __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$codeintel$2f$query$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["QueryEngine"](repo.symbolGraph);
    const ctx = {
        repo,
        qe
    };
    ...
    const reports = [];
    let all = [];
    for (const s of __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$agents$2f$specialists$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["SPECIALISTS"]){
        let findings = [];
        try {
            findings = s.run(ctx);
        } catch  {
...
function projectScore(current, buckets) {
...
function summarize(agent, findings) {
...
function planSummary(all, buckets, cur, proj) {
...
"[project]/src/app/api/repos/[id]/agents/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$agents$2f$orchestrator$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/agents/orchestrator.ts [app-route] (ecmascript)");
;
;
;
...
async function GET(_req, { params }) {
    ...
    try {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json((0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$agents$2f$orchestrator$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["runSwarm"])(repo));
    } catch (e) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1n9xogp._.js.map